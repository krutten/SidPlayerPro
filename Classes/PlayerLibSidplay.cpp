...
sid_fc_t   sDefault8580[sDefault8580PointCount] =
{
	{ 0, 0        },
	{ 64, 400	  },
	{ 128, 800    },
...
// ----------------------------------------------------------------------------
const char* PlayerLibSidplay::getAuthor()
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
const char* PlayerLibSidplay::getReleaseInfo()
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
{
	...
	cfg.playback	  = sid2_mono; //FIXME: Needs to be changed if we go stereo
...
//	cfg.sampleFormat  = SID2_BIG_UNSIGNED;
	...
	while ( int rc = mSidEmuEngine->config(cfg) != 0 )
		printf("configure error: %s -- retrying...\n", mSidEmuEngine->error());
	
	printf( "*** cfg.sidModel=%d, cfg.sidDefault=%d [OUR CONFIG]\n", cfg.sidModel, cfg.sidDefault );
...
// ----------------------------------------------------------------------------
{
	...
	int rc = loadTuneFromBuffer(buffer, length, subtune, settings);
...
// ----------------------------------------------------------------------------
void PlayerLibSidplay::startPrevSubtune()
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
void PlayerLibSidplay::startNextSubtune()
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
void PlayerLibSidplay::fillBuffer(void* buffer, unsigned int len)
// ----------------------------------------------------------------------------
{
	if (mSidEmuEngine == NULL)
		return;
