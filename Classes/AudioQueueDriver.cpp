...
// ----------------------------------------------------------------------------
{
	...
	err = AudioSessionInitialize( CFRunLoopGetCurrent(), NULL, AudioSessionInterruptionCallback, self );
...
#if 0
		err = AudioSessionAddPropertyListener( kAudioSessionProperty_CurrentHardwareOutputVolume, AudioSessionPropertyChangeCallback, NULL );
		if (err)
...
void AudioQueueDriver::AudioSessionPropertyChangeCallback(void  *inClientData,
									   ...
									   const void                *inData )
{
	fprintf(stderr, "FIXME: handle audio session property change for ID %d\n", inID);
}
...
{
	fprintf(stderr, "FIXME: handle audio queue property change for ID %d\n", inID);
}
...
#if 0 // this doesn't quite work as expected
